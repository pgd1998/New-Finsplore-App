 ...
 * 
 * Optimized for handling thousands of transactions per user with proper
 * indexing.
...
public interface TransactionRepository
       ...
       /**
        * Finds transactions that need user categorization (AI suggested but not
        * confirmed)
        */
       ...
       List<Transaction> findLowConfidenceTransactions(@Param("userId") Long userId,
        ...
        */
       @Query("SELECT t FROM Transaction t WHERE t.user.id = :userId ORDER BY t.transactionDate DESC, t.createdAt DESC")
       List<Transaction> findMostRecentByUserId(@Param("userId") Long userId, Pageable pageable);

       /**
        * Helper method to get the single most recent transaction
        */
       default Optional<Transaction> findSingleMostRecentByUserId(Long userId) {
              List<Transaction> transactions = findMostRecentByUserId(userId, PageRequest.of(0, 1));
        ...
        */
       @Modifying
       @Query("UPDATE Transaction t SET t.category = :category, t.isCategorizedByUser = true, t.lastCategorizedAt = :currentTime WHERE t.id IN :transactionIds AND t.user.id = :userId")
       ...
       int markTransactionsAsRecurring(@Param("userId") Long userId,
                     @Param("transactionIds") List<String> transactionIds);
       ...
       Optional<Transaction> findByIdAndUserId(@Param("transactionId") String transactionId,
       ...
       List<Transaction> searchTransactionsByQuery(@Param("userId") Long userId, @Param("query") String query,
                     Pageable pageable);
